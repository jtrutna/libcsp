 ...
 **/
#define MULTIKISS_CRC32   1
#define MULTIKISS_MAX_IFS 2
#define MULTIKISS_MTU     256
...
volatile short csp_if_multikiss0_transmitting = 0;
volatile short csp_if_multikiss1_transmitting = 0;
...
csp_iface_t multikiss_ifs[MULTIKISS_MAX_IFS] = {
	{
		.name = "MKISS0",
		.nexthop = csp_multikiss_tx0,
		.mtu = MULTIKISS_MTU,
	}, {
		.name = "MKISS1",
		.nexthop = csp_multikiss_tx1,
		.mtu = MULTIKISS_MTU,
	},
};
...
csp_iface_t * csp_multikiss_get_if(uint8_t handle) {
	if (handle >= MULTIKISS_MAX_IFS) {
		return NULL;
	}

	return &multikiss_ifs[handle];
}
...
int csp_multikiss_tx0(csp_packet_t * packet, uint32_t timeout) {
	return csp_multikiss_tx(0, packet, timeout);
}

int csp_multikiss_tx1(csp_packet_t * packet, uint32_t timeout) {
	return csp_multikiss_tx(1, packet, timeout);
}
...
void csp_multikiss_rx0(uint8_t * buf, int len, void * pxTaskWoken) {
	csp_multikiss_rx(0, buf, len, pxTaskWoken);
}

void csp_multikiss_rx1(uint8_t * buf, int len, void * pxTaskWoken) {
	csp_multikiss_rx(1, buf, len, pxTaskWoken);
}

int csp_multikiss_init(uint8_t handle, csp_multikiss_putstr_f kiss_putstr_f,
	csp_multikiss_discard_f kiss_discard_f)
{
	if (handle >= MULTIKISS_MAX_IFS) {
	...
	/* Regsiter interface */
	csp_route_add_if(&multikiss_ifs[handle]);
